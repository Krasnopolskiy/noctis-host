...
func dispatch(msg *incomingMsg, sup *singboxSupervisor, logger *log.Logger) ack {
	switch msg.Type {
	case "hello":
		return ack{
			ID:   msg.ID,
	...
	case "start":
		...
		port, err := sup.start(args.Config)
		if err != nil {
	...
	case "reload":
		...
		port, err := sup.reload(args.Config)
		if err != nil {
		...
		return ack{ID: msg.ID, Type: "ack", OK: true, Data: map[string]int{"socksPort": port}}
	case "stats":
		return ack{ID: msg.ID, Type: "ack", OK: true, Data: map[string]any{"stub": true}}
	default:
		return ack{
			ID:    msg.ID,
			Type:  "ack",
